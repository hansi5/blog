...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("You have already signed up, log in instead.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(email=email).first()
        if not user:
            flash("This email does not found, please try again")
            return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user:
            ...
            db.session.commit()
            return redirect(url_for("get_all_posts"))
    return render_template("post.html", post=requested_post, user=current_user, form=form, comment=comments)
